...
export class <%= h.inflection.camelize(name) %>Service {
  ...
  async findAll(paginationOptions?: IPaginationOptions, search?: string): Promise<{ items: <%= h.inflection.camelize(h.inflection.singularize(name)) %>[]; meta: PaginationMetaDto }> {
    return this.repository.findManyWithPagination({ paginationOptions: paginationOptions || { page: 1, limit: 10 }, search });
  }
  ...
  async create(dto: Create<%= h.inflection.camelize(h.inflection.singularize(name)) %>Dto): Promise<<%= h.inflection.camelize(h.inflection.singularize(name)) %>> {
