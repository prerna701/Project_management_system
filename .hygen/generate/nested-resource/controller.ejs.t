...
export class <%= h.inflection.camelize(name) %>Controller {
  ...
  @Get('<%= h.inflection.dasherize(h.inflection.underscore(parent)) %>/<%= h.inflection.dasherize(h.inflection.underscore(parent)) %>Id/<%= h.inflection.dasherize(h.inflection.underscore(name)) %>')
  @SetMetadata('abilities', [['browse', '<%= h.inflection.underscore(name) %>']])
  @UseGuards(CaslAbilityGuard)
  @HttpCode(HttpStatus.OK)
  async findBy<%= h.inflection.camelize(h.inflection.singularize(parent)) %>(
    @Param('<%= h.inflection.camelize(h.inflection.singularize(parent), true) %>Id') <%= h.inflection.camelize(h.inflection.singularize(parent), true) %>Id: string,
    @Query() query: BaseQueryDto,
  ) {
    const { paginationOptions } = extractQueryOptions(query, API_PAGE_LIMIT);
    const { items, meta } = await this.service.findBy<%= h.inflection.camelize(h.inflection.singularize(parent)) %>Id(<%= h.inflection.camelize(h.inflection.singularize(parent), true) %>Id, paginationOptions, query.search);
    return createPaginatedResponse('<%= h.inflection.humanize(name) %> fetched successfully', items, meta);
  }

  @Get('<%= h.inflection.dasherize(h.inflection.underscore(name)) %>/:id')
  ...
  @Post('<%= h.inflection.dasherize(h.inflection.underscore(parent)) %>/<%= h.inflection.dasherize(h.inflection.underscore(parent)) %>Id/<%= h.inflection.dasherize(h.inflection.underscore(name)) %>')
  @SetMetadata('abilities', [['add', '<%= h.inflection.underscore(name) %>']])
  @UseGuards(CaslAbilityGuard)
  @HttpCode(HttpStatus.CREATED)
  async create(
    @Param('<%= h.inflection.camelize(h.inflection.singularize(parent), true) %>Id') <%= h.inflection.camelize(h.inflection.singularize(parent), true) %>Id: string,
    @Body() dto: Create<%= h.inflection.camelize(h.inflection.singularize(name)) %>Dto,
  ) {
    const item = await this.service.create(<%= h.inflection.camelize(h.inflection.singularize(parent), true) %>Id, dto);
  ...
  @Patch('<%= h.inflection.dasherize(h.inflection.underscore(name)) %>/:id')
  ...
  @Delete('<%= h.inflection.dasherize(h.inflection.underscore(name)) %>/:id')
